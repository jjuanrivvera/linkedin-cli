...
// localGroups are command subtrees that never talk to the LinkedIn API (setup/meta/local).
...
// annotation. The `api` escape hatch is GET-ONLY (this is a read-only client), so it is
// annotated kindRead and classified like any other read — no method gating is needed.
// Unannotated leaves classify as DESTRUCTIVE, never allowed (fail-closed, §3b #4).
func classifyTree() []apiCmdInfo {
	...
	walk = func(cmd *cobra.Command, names [][]string) {
		for _, child := range cmd.Commands() {
			...
			if len(names) == 0 && slices.Contains(localGroups, child.Name()) {
				continue
			}
			if child.Runnable() {
...
func init() {
	metaRegistrars = append(metaRegistrars, func(_ *deps) *cobra.Command {
		agentCmd := &cobra.Command{
			...
			Long:  "Generate safety configuration for AI agents that drive linkedin.",
		}
...
denied.`,
			Example: `  linkedin agent guard --host claude-code
...
func writeClaudeCodeFiles(cmd *cobra.Command, cls classification) error {
	...
	files := map[string]string{
		".claude/hooks/linkedin-guard.sh":      hook,
		".claude/linkedin-guard.settings.json": settings,
	}
	...
	fmt.Fprintln(cmd.ErrOrStderr(), "merge .claude/linkedin-guard.settings.json into .claude/settings.json to activate the hook")
	return nil
}
...
func bashPattern(path string) string { return "Bash(linkedin " + path + ":*)" }

// mcpToolPattern is the MCP tool name a host gates: linkedin_<group>_<verb> under the linkedin
...
func mcpToolPattern(path string) string {
	return "mcp__linkedin__linkedin_" + strings.ReplaceAll(path, " ", "_")
}
