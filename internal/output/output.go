// Package output renders LinkedIn API JSON in table/json/yaml/csv/id. One renderer serves
...
// preferredOrder lists LinkedIn fields that should lead a table when present, before the
// rest fall back to alphabetical. Deterministic ordering is required by GOAL.md §11.
var preferredOrder = []string{
	"id", "title", "jobPostingTitle", "company", "companyName", "name", "universalName",
	"location", "formattedLocation", "workplaceType", "workRemoteAllowed", "listedAt",
	"employmentStatus", "companyApplyUrl", "displayName", "geoId", "staffCount", "description",
}
