# Makefile — linkedin (LinkedIn CLI), built to the cliwright standard.
# `make verify` is the deterministic acceptance gate; `make accept` adds the LLM judge.
BINARY      := linkedin
MODULE      := github.com/jjuanrivvera/linkedin-cli
...
COVERAGE_MIN ?= 80
# spec-completeness: min % of the enumerated LinkedIn public API the manifest must wrap
API_COVERAGE_MIN ?= 90
